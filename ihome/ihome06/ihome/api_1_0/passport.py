...
def login():
    ...
    # 如果用户存在 并且 错误次数大于等于5
    if user_error_count is not None and int(user_error_count) >= constants.LOGIN_ERROR_MAX_NUM:
    ...
    if user is None or not user.check_password_hash(password):
        ...
        try:
            ...
            redis_store.expire('user_error_count_%s' % user_ip, constants.LOGIN_ERROR_FORBID_TIME)
        except Exception as e:
            logging.error(e)
